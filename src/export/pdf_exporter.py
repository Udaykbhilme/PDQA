...
class PDFExporter:
    ...
    def _create_timetable(self, assignments, info):
        time_slots = self._generate_time_slots(info)
    ...
    def _generate_time_slots(self, info: Dict):
    ...
    def _pdf_cell(self, assignments, day, start_time):
        """Return plain text for cell in PDF table."""
        match = [a for a in assignments if a.day == day and a.start_time == start_time]
        if not match:
            return ""
        a = match[0]
        subj, fac, ven = getattr(a, "subject", None), getattr(a, "faculty", None), getattr(a, "venue", None)
        if not subj or not fac or not ven:
            return "Data Missing"
        return f"{subj.code}\n{subj.name}\n{fac.name} ({fac.faculty_code})\n{ven.name}"

    def _html_cell(self, assignments, day, start_time):
        """Return HTML cell with proper color class."""
        match = [a for a in assignments if a.day == day and a.start_time == start_time]
        if not match:
            return "<td></td>"
        a = match[0]
        ...
        return f"<td class='{cell_class}'>{subj.code}<br>{subj.name}<br>{fac.name} ({fac.faculty_code})<br>{ven.name}</td>"
